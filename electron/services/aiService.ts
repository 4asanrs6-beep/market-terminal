...
- 箇条書きとセクション分けを活用
- 1000-1500文字程度（カバー銘柄が多い場合は2000文字まで可）
- 投資推奨は行わない（分析と事実のみ）
...
- 【最重要】全ての大きな値動き（当日±2%以上、5日間±5%以上）について、必ず「なぜ」を具体的に説明すること
  - 値動きの数字だけを列挙するブリーフィングは価値がない。原因・背景の説明こそが本レポートの核心
  - マクロ要因（金利動向、FRB政策、インフレ指標、地政学リスク等）
  - 個別要因（決算、ガイダンス、アナリスト格付変更、M&A、規制、供給問題、天候等）
  - テクニカル要因（出来高急増、サポート/レジスタンス、短期過熱感等）
  - コモディティは特に供給・需要のファンダメンタルズ要因（産地の天候、在庫水準、OPEC動向、作付面積等）を必ず言及すること
  - ニュースが提供されている場合はそれを根拠に説明し、ない場合でも一般的な市場知識から背景を推論して説明すること（「理由不明」で済ませない）
